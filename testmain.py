...
model = load_model(f'my_model_{selected_language}')

# Create an instance of the Language Tool grammar correction tool
parser = language_tool_python.LanguageTool('en-US')
...
with mp.solutions.holistic.Holistic(min_detection_confidence=0.75, min_tracking_confidence=0.75) as holistic:
    ...
    while cap.isOpened():
        ...
        if keyboard.is_pressed('enter'):
            ...
            matches = parser.check(text)
            # Extract the corrected result from the parsed grammar
            grammar_result = language_tool_python.utils.correct(text, matches)
